...
describe('LoginForm', () => {
  ...
  it('submits valid values', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Sign in' }))
    // vee-validate's handleSubmit validates asynchronously (even with a
    // synchronous zod schema, the pipeline is promise-based) — the emit
    // lands a tick after the click resolves, confirmed by probing a raw
    // form (no vee-validate) where the same click/emit pattern was
    // synchronous. waitFor is needed here, not just `await user.click`.
    await waitFor(() => {
